...
- 将 429、HTTP、超时、网络、模型响应、业务 JSON 和 Schema 错误分类；
- 对 429 执行有限次数的退避重试，并优先遵守 `Retry-After`；
- 在批量评测的相邻案例之间节流，降低触发 RPM 限制的概率。

当前已经完成提示词版本对比基础设施、429 有限重试和请求节流。修复提示词与业务降级策略将在下一阶段补充。
...
批量评测默认在相邻案例之间等待 6500 ms，并对 429 最多额外重试 2 次。可以在 `.env` 中调整：
...
`EVAL_MAX_RETRIES=2` 表示首次请求之外最多再尝试 2 次，总尝试次数最多为 3。普通的 400、401 等错误不会重试。
...
案例包括 4 条正常文章、3 条边界文章和 3 条包含干扰指令的对抗文章。执行前请确认 `.env`、模型额度和最大输出 token 配置。评测按顺序并节流执行，单个案例失败不会中断其余案例。由于存在等待与 429 重试，实际 HTTP 尝试次数可能大于 20。
...
报告会额外记录请求是否成功、错误分类和尝试次数。JSON、Schema 等输出指标只以请求成功的案例为分母，避免把 429 错误算成提示词质量问题。
